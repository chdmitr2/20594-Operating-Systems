/* Chudnovsky Dmitriy                                             /
/  Id 324793900                                                   /
/  To add code in specifical places that to support commands like /
/  jobs,fg,bg and respond to job control in this shell           */
...
int runCommand(struct job newJob, struct jobSet * jobList, 
	...
	int job_N;
    	...
    	job_N = atoi(newJob.progs[0].argv[1] + 1);
            ...
            if(job->jobId == job_N)
    	      {
               ...
               if(newJob.progs[0].argv[0][0] == 'f')
    	         {
    	          /*put the job you found in the foreground (use tcsetpgrp) */
    		  jobList->fg = job;                    
    		  if (tcsetpgrp(0, job->pgrp)) perror("tcsetpgrp failed");
    		 }
               /* Resume running the job*/
    	       if(kill(-job->pgrp, SIGCONT)) perror("job failed");
        ...
        /*syntax check failed or job not found return 1*/
        return 1;
    }
